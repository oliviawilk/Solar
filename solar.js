...
function processSitesWatts(results) {
	...
	// var data = createJsons(wattsLabel, wattsData);
// 	makeLineGraphDailyWatts(data);
...
function displayAllSiteTodayWatts(data){
	...
	data.forEach(function(site) {
		...
		if (whr > 0) {
			...
			whrs.push(whr);
			totalwattstoday.push(wattsList);
		}
		
	});
	makeSumSummaryGraph(names,whrs);
	makePieSumGraph(names,whrs);
	//makeLineSummaryGraph(names,whrs);
	//console.log('Below is totalwattstoday');
	//console.log(JSON.stringify(totalwattstoday))
	var graphs= createJsons(names, totalwattstoday);
    makeLineGraphDailyWatts(graphs);
}
...
{
  document.querySelector('#output3').innerHTML += "<h1> Hourly Kilowatts for Each School</h1>";
...
//Making the data for the composite Line Chart
function createJsons(names, allWatts){
	Jsons=[];
	for (let i=0; i<names.length; i++){
		let graph={label:names[i], data: allWatts[i]};
		Jsons.push(graph);
	}
	//console.log('Below is Jsons');
	//console.log(JSON.stringify(Jsons));
	return Jsons;
}
//Creating a pie chart of the total proportion of watts from each school
function makePieSumGraph(names,watts) {
  document.querySelector('#output4').innerHTML += "<h1> Total Watts Produced Today</h1>";
